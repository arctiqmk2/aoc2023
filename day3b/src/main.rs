...
fn main() -> io::Result<()> {
    ...
    let mut found_gears: HashMap<(usize, usize), Vec<i32>> = HashMap::new();
    let mut total_gear_ratio_sum = 0;
        ...
        for y in y_start_adj..=y_end_adj {
            for x in x_start_adj..=x_end_adj {
                ...
                if symbol != '.' && !symbol.is_digit(10) {
                    ...
                    if symbol == '*' {
                        if !found_gears.contains_key(&(x, y)) {
                            found_gears.insert((x, y), Vec::new());
                        }
                        found_gears.get_mut(&(x, y)).unwrap().push(info.number);
                    }
                    break;
                }
            }
    ...
    // Output the found gears and their adjacent numbers
    for ((x, y), numbers) in &found_gears {
        println!("Gear at ({}, {}): {:?}", x, y, numbers);
        if numbers.len() == 2 {
    ...
    println!("Gear Ratio Sum: {}", total_gear_ratio_sum);
