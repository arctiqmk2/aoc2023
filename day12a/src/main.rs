...
fn main() {
    ...
    lines.iter().for_each(|line| {
        ...
        let springs: Vec<u8> = parts[0].as_bytes().to_vec();
        let numbers: Vec<i64> = parts[1].split(',').filter_map(|s| s.trim().parse().ok()).collect();
        ...
        total += score_sets(cached_sets, springs, numbers, 0, 0, 0)
    });
    println!("total: {:?}", total);
}
