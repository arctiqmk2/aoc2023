...
fn main() {
    ...
    let mut total_calculated_distance = 0;

    galaxy_pairs.iter().for_each(|[near_galaxy, distant_galaxy]| {
        let x_dist = (near_galaxy.0 - distant_galaxy.0).abs();
        let calculated_x_dist = calculate_range(near_galaxy.0, distant_galaxy.0).iter().map(|x| expand_x[*x as usize]).sum::<i32>();
        let y_dist = (near_galaxy.1 - distant_galaxy.1).abs();
        let calculated_y_dist = calculate_range(near_galaxy.1, distant_galaxy.1).iter().map(|y| expand_y[*y as usize]).sum::<i32>();
